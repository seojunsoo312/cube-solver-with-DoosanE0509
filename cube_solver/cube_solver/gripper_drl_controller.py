...
class GripperController:
    ...
    def _send_drl_script(self, code: str, desc: str = "gripper command") -> bool:
        if not self.cli.service_is_ready():
            self.node.get_logger().error("DRL service not ready; skip gripper command.")
            return False
        ...
        while (not future.done()) and rclpy.ok():
            if (time.monotonic() - t0) > self.timeout_sec:
                self.node.get_logger().error(
                    f"Gripper DRL '{desc}' timeout ({self.timeout_sec}s)."
                )
                return False
            time.sleep(0.01)
        ...
        try:
            ...
            if result is None or not result.success:
                self.node.get_logger().warn(f"Gripper DRL '{desc}' failed or no response.")
                return False
            self.node.get_logger().debug(f"Gripper DRL '{desc}' succeeded.")
            return True
        except Exception as exc:
            self.node.get_logger().error(f"Service call failed for '{desc}': {exc}")
            return False
    ...
    def initialize(self) -> bool:
        ...
        success = self._send_drl_script(init_script, "initialize")
        if success:
    ...
    def move(self, stroke: int) -> bool:
        ...
        success = self._send_drl_script(move_script, f"move {stroke}")
        if success:
            self.node.get_logger().info(f"Gripper move command for {stroke} sent.")
        else:
            self.node.get_logger().error("Failed to enqueue gripper move command.")
        return success

    def terminate(self) -> bool:
        self.node.get_logger().info("Terminating gripper connection...")
        terminate_script = "flange_serial_close()"
        success = self._send_drl_script(terminate_script, "terminate")
        if success:
